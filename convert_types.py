...
class Init:
	...
	def convert_to_int(self, x, def_val):
		try:
			return int(round(x))
		except:
			return def_val
	...
	def outer(self):
		...
		try:
			...
			defval = int(self.default_value_entry.get())
			...
			tkMessageBox.showerror("ConvertTypes", "Complete!")
			
		except:
			exc_type, exc_value, exc_trace = sys.exc_info()
			lines = traceback.format_exception(exc_type, exc_value, exc_trace)
			tkMessageBox.showinfo("Error", "".join(line for line in lines))
	...
	def go(self, in_dat_path, out_dir, def_val):
		...
		with open(in_dat_path, "rb") as fin, open(out_dat_path, "wb") as fout:
			while True:
				...
				# attempt to convert the data to the desired type
				try:
					s = struct.pack(out_code * len(t), *t)
				except:
					# if converting to an integral type use a default
					# value for all non-convertible types
					if self.out_type_is_integral(out_type):
						print("failed: manually convert")
						t = tuple(self.convert_to_int(x, def_val) for x in t)
						s = struct.pack(out_code * len(t), *t)
					else:
						raise
				fout.write(s)
